...
CREATE OR REPLACE FUNCTION spec.smpeventdeadline
(
...
--    create table IF NOT EXISTS public.tmpProcessStatus
    ...
    from "SHOE_SAMPLE_REQUEST" po
    JOIN "SHOE_SAMPLE_REQUEST_SKU" po_sku ON ( po_sku.cd_shoe_sample_request = po.cd_shoe_sample_request )
    JOIN "SHOE_SAMPLE_REQUEST_SKU_PROCESS" po_sku_prc ON ( po_sku_prc.cd_shoe_sample_request_sku = po_sku.cd_shoe_sample_request_sku )
 ...
 GROUP BY i.ds_shoe_process_item,
    ...
    as select p.ds_shoe_process_item,
           ...
           ( CASE WHEN p.fl_finalized = 'Y' THEN v_label_done
                  ELSE CASE 
                           WHEN b.ds_shoe_process_status IS NOT NULL THEN b.ds_shoe_process_status 
                           ELSE v_label_on_time 
                         END
            END ) as ds_info, 
           ( CASE WHEN p.fl_finalized = 'Y' THEN '00c0ef'
                  ELSE CASE 
                           WHEN b.ds_shoe_process_status IS NOT NULL THEN b.ds_background_color 
                           ELSE '337ab7' 
                         END
           ...
           ( CASE WHEN p.fl_finalized = 'Y' THEN '' 
                  ELSE CASE 
                           WHEN b.ds_shoe_process_status IS NOT NULL THEN b.ds_font_color 
                           ELSE '' 
                         END
           ...
           ( CASE WHEN p.fl_finalized = 'Y' THEN 999999999 
                  ELSE CASE 
                           WHEN b.ds_shoe_process_status IS NOT NULL THEN b.nr_days_before_deadline 
                           ELSE null 
                         END
 ...
 LEFT JOIN retProcessDeadlineStatus( dt_dead_line, cd_shoe_process_item_group) b ON (1 = 1)
;
    ...
    order by ds_info, ds_background_color ) as r;
