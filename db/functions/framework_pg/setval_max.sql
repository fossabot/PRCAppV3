...
CREATE OR REPLACE FUNCTION public.setval_max
(
    schema_name name,
...
BEGIN
    ...
    FOR sql_code IN
        --SELECT 'SELECT SETVAL(' ||quote_literal(N.nspname || '."' || S.relname || '"' )|| ', coalesce(MAX(' ||quote_ident(C.attname)|| '),0) + 1 ) FROM ' || quote_ident(N.nspname) || '.' || quote_ident(T.relname)|| ';' AS sql_code
        SELECT 'SELECT SETVAL(' ||quote_literal(N.nspname || '."' || S.relname || '"' )|| ', ( coalesce((select MAX(' ||quote_ident(C.attname)|| ') FROM ' || quote_ident(N.nspname) || '.' || quote_ident(T.relname)|| '),0) + 1 ));' AS sql_code
