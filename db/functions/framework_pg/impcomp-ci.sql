...
CREATE OR REPLACE FUNCTION public.impcompprocci()
  RETURNS void    AS
$$
DECLARE 
r record;
...
BEGIN
    ...
    vweb      = null;
     ...
     SELECT cd_unit_measure
       INTO vunit
       FROM "UNIT_MEASURE" 
     WHERE ds_unit_measure = q.ds_j;
     
    if isnumeric(q.ds_g ) THEN
        vgrade = q.ds_g;
    end if;

    if strpos(q.ds_k, 'http')> 0 THEN
        vweb = q.ds_i;
        ELSE
        vremarks = q.ds_k;
    END IF;
...
END
$$  LANGUAGE plpgsql;
ALTER FUNCTION public.impcompprocci() SET search_path=pg_catalog, public, rfq;