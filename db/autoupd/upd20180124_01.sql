...
ALTER TABLE `hrms`.`EMPLOYEE_X_BANK_BRANCH` 
ADD COLUMN `fl_default` CHAR(1) NOT NULL DEFAULT 'N' AFTER `cd_currency`;

INSERT INTO `hrms`.`SYSTEM_PARAMETERS` (`ds_system_parameters`, `ds_system_parameters_id`, `ds_system_parameters_obs`, `ds_system_parameters_value`) VALUES ('Code do RMB', 'CURRENCY_RMB_CODE', 'Code do RMB', '3');


drop function if exists getBankInfoPersonal;

DELIMITER //

CREATE FUNCTION getBankInfoPersonal 
(
    v_cd_employee integer
)
RETURNS varchar(200)
...
BEGIN
   DECLARE v_result varchar (200);
    ...
    SELECT cd_employee_x_bank_branch 
           || '<#>' ||
           ( ( select ds_bank FROM BANK WHERE cd_bank =  BANK_BRANCH.cd_bank) || '/' || BANK_BRANCH.ds_bank_branch  || '/' || EMPLOYEE_X_BANK_BRANCH.ds_account_number ) 
           || '<#>' ||
           COALESCE(cd_currency, -1)
           || '<#>' ||
           COALESCE( ( select ds_currency from CURRENCY where cd_currency = EMPLOYEE_X_BANK_BRANCH.cd_currency ), '')

   INTO v_result
    FROM EMPLOYEE_X_BANK_BRANCH 
    JOIN BANK_BRANCH ON (BANK_BRANCH.cd_bank_branch = EMPLOYEE_X_BANK_BRANCH.cd_bank_branch)
    WHERE EMPLOYEE_X_BANK_BRANCH.cd_employee = v_cd_employee
      AND EMPLOYEE_X_BANK_BRANCH.fl_default = 'Y';
...
ADD COLUMN `cd_human_resource_header` INT NULL AFTER `dt_record`,
ADD INDEX `FKDEPARTMENT001_idx` (`cd_human_resource_header` ASC);
ALTER TABLE `hrms`.`DEPARTMENT` 
ADD CONSTRAINT `FKDEPARTMENT001`
  FOREIGN KEY (`cd_human_resource_header`)
  REFERENCES `hrms`.`HUMAN_RESOURCE` (`cd_human_resource`)
  ON DELETE RESTRICT
  ON UPDATE RESTRICT;


update SYSTEM_MENU set cd_system_menu_parent = 1040, nr_order = 100 where  ds_system_menu = 'Department';
