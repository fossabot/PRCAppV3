-- View: spec."vSHOE_SPECIFICATION_COMPONENT"

-- DROP VIEW spec."vSHOE_SPECIFICATION_COMPONENT";

CREATE OR REPLACE VIEW spec."vSHOE_SPECIFICATION_COMPONENT" AS 
 SELECT c.cd_shoe_specification_component,
    c.cd_shoe_specification,
    ...
    a.cd_shoe_specification_component_attributes,
    ...
    s.cd_system_product_category

   FROM "SHOE_SPECIFICATION_COMPONENT" c
     JOIN "SHOE_SPECIFICATION" s ON s.cd_shoe_specification = c.cd_shoe_specification
     JOIN "SHOE_LEVEL_X_PRODUCT_COMPONENT" l ON ( l.cd_shoe_level_x_product_component = c.cd_shoe_level_x_product_component AND l.cd_system_product_category = s.cd_system_product_category )
     LEFT JOIN "SHOE_SPECIFICATION_COMPONENT_ATTRIBUTES" a ON a.cd_shoe_specification_component = c.cd_shoe_specification_component
  ...
  WHERE sl.dt_deactivated IS NULL AND l.dt_deactivated IS NULL AND l.cd_shoe_level = 2;

ALTER TABLE spec."vSHOE_SPECIFICATION_COMPONENT"
  OWNER TO postgres;
GRANT ALL ON TABLE spec."vSHOE_SPECIFICATION_COMPONENT" TO postgres;
GRANT SELECT ON TABLE spec."vSHOE_SPECIFICATION_COMPONENT" TO "mbReport";
GRANT ALL ON TABLE spec."vSHOE_SPECIFICATION_COMPONENT" TO devshoesdemorole;
