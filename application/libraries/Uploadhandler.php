...
class uploadhandler
{
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                ...
                if ($this->options['addToDocFile']) {
                    $this->addToDocFile($file);
                }
    ...
    protected function addToDocFile(&$file) {
        $this->CI = & get_instance();
        $this->CI->load->library ('cdocrep');
        //die ($file->name);
        $code = $this->CI->cdocrep->addToRepositoryFile($this->options['upload_dir'] . $file->name, -1, true);
        
        $file->cd_document_file = $code ;
        $file->cd_document_repository_type = $this->CI->cdocrep->typeInfo['recid'];
