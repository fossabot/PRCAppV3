...
class ExchangeClient {
    ...
    public function get_events($start, $end) {
        ...
        //if we only returned one event, then it doesn't send it as an array, just as a single object. so put it into an array so that everything works as expected.
     ...
     * @param string $folder . (default:"inbox", or options:[sentitems,contacts,msgfolderroot], see:https://docs.microsoft.com/zh-cn/exchange/client-developer/web-service-reference/distinguishedfolderid)
     * @param bool $folderIdIsDistinguishedFolderId . (default: true, is $folder a DistinguishedFolderId like 'msgfolderroot' or a simple FolderId like 'AAAdAFRUSS5NaWx3YXVrZWVMTVN...')
     * @return array|bool $messages (an array of objects representing the messages)
     */
    public function get_messages($limit = 50, $onlyunread = false, $folder = "inbox", $folderIdIsDistinguishedFolderId = true) {
        ...
        //if we only returned one message, then it doesn't send it as an array, just as a single object. so put it into an array so that everything works as expected.
        if (!is_array($items)) $items = array($items);
        ...
        foreach ($items as $item) {
            ...
            if (!isset($messageobj->Body) || $onlyunread && $messageobj->IsRead) continue;
     ...
     */
    public function send_message($to, $subject, $content, $bodytype = "Text", $saveinsent = true, $markasread = true, $attachments = [], $cc = false, $bcc = false) {
        $this->setup();
     ...
     * @throws Exception
     * @access public
     * @param object $ItemId (such as one returned by get_messages)
     ...
     * @throws Exception
     * @access public
     * @param object $ItemId (such as one returned by get_messages, has Id and ChangeKey
     * @param object $FolderId (such as one returned by get_messages, has Id and ChangeKey)
     * @return object The new ItemId (such as one returned by get_messages, has Id and ChangeKey)
     */
    public function move_message($ItemId, $FolderId) {
        ...
        $MoveItem->ToFolderId->FolderId->Id = $FolderId->Id;
        ...
        if ($response->ResponseMessages->MoveItemResponseMessage->ResponseCode == "NoError") {
            ...
            return $this->lastError = $response->ResponseMessages->MoveItemResponseMessage->ResponseCode;
        }
    }
    ...
    public function get_subfolders($ParentFolderId = "inbox", $Distinguished = TRUE) {
        ...
        if ($response->ResponseMessages->FindFolderResponseMessage->ResponseCode == "NoError") {
            ...
            return $this->lastError = $response->ResponseMessages->FindFolderResponseMessage->ResponseCode;
        }
    }
