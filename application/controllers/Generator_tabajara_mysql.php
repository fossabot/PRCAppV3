...
class generator_tabajara_mysql extends controllerBasicExtend {
    ...
    public function index() {
        ...
        $this->load->view("generator_tabajara_mysql_view", $send);
    }
...
select c.column_name, 
       ('ds' || substr(c.column_name, 3, 999)) AS  column_name_desc , 
       c.column_default, 
       c.data_type,
 ...
 LEFT OUTER JOIN (SELECT distinct tc.table_name,
    ...
    ( SELECT x.column_name
          ...
          WHERE x.table_name = kcu.referenced_table_name AND (x.data_type like '%char%'  OR x.data_type like '%text%' ) order by x.ordinal_position LIMIT 1) AS foreigner_desc_column
   ...
   public function index() {
      ...
      ';
        ...
        foreach ($infotable as $key => $value) {
            ...
            if ($setVlr == 'Y') {
                $vlrNormal = ' value="<?php echo($'.$column_name.')?>" fieldname="'.$column_name.'"';
                $vlrPL     = ' plcode="<?php echo($'.$column_name.')?>"  value="<?php echo($'.$column_name_desc.')?>" fieldname="'.$column_name_desc.'"';
            } else {
            ...
            $fupper = '$f->retTypeStringUpper()';
            $fPLText = '$f->retTypeStringUpper()';
            ...
            if (!is_null($foreign_table_name)) {
                ...
                $infoFormBootstrap = $infoFormBootstrap . '
                        ...
                        <label for="' . $formField . '" class="col-sm-1 control-label "><?php echo($' . $transField . ') ?>:</label>
                        <div class="col-sm-2">
                            <input type="text" class="form-control input-sm" id="' . $formField . '" '.$vlrPL.' mask="PL" model = "<?php echo ($this->encodeModel(\'' . $foreig_model . '\')); ?>" fieldname="' . $column_name_desc . '" code_field="' . $column_name . '"  relid="-1" relCode ="-1" type="text">
                        </div>';
            ...
            if ((stripos($data_type, 'char') !== FALSE ) || (stripos($data_type, 'text') !== FALSE )) {
                ...
                $infoFormBootstrap = $infoFormBootstrap . '
                    ...
                    <label for="' . $formField . '" class="col-sm-1 control-label "><?php echo($' . $transField . ') ?>:</label>
                    <div class="col-sm-2">
                        <input type="text" class="form-control input-sm" id="' . $formField . '" '.$vlrNormal.' mask="C" type="text" maxlength="' . $character_maximum_length . '" >
                    </div>';
            ...
            if (stripos($data_type, 'int') !== FALSE) {
                ...
                $infoFormBootstrap = $infoFormBootstrap . '
                    ...
                    <label for="' . $formField . '" class="col-sm-1 control-label "><?php echo($' . $transField . ') ?>:</label>
                    <div class="col-sm-2">
                        <input type="text" class="form-control input-sm" id="' . $formField . '" '.$vlrNormal.' mask="I" >
                    </div>';
            ...
            // definicoes do grid!!
            if (stripos($data_type, 'num') !== FALSE || stripos($data_type, 'dec') !== FALSE) {
                $formField = $column_name . $formSufix;
                ...
                $infoFormBootstrap = $infoFormBootstrap . '
                    ...
                    <label for="' . $formField . '" class="col-sm-1 control-label "><?php echo($' . $transField . ') ?>:</label>
                    <div class="col-sm-2">
                        <input type="text" class="form-control input-sm" id="' . $formField . '"     '.$vlrNormal.' mask="N;' . $numeric_precision . '.' . $numeric_scale . '" >
                    </div>';
