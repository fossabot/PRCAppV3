...
class project_comments extends controllerBasicExtend {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model("tti/project_comments_model", "mainmodel", TRUE);
    ...
    public function index() {
        ...
        $fm->addPickListFilter('Project', 'filter_1', 'tti/project', '"PROJECT_COMMENTS".cd_project');
        $fm->addPickListFilter('Human Resource', 'filter_2', 'human_resource', '"PROJECT_COMMENTS".cd_human_resource');
        $fm->addSimpleFilterUpper('Comments', 'filter_3', '"PROJECT_COMMENTS".ds_comments');
    ...
    public function openCommentPL($cd_project_model, $cd_project_comment = -1, $cd_project_comments_answer = -1) {
        ...
        $dataAnswer = $this->mainmodel->retRetrieveArray(" WHERE cd_project_comments = $cd_project_comments_answer");



        if ($cd_project_comment == -1) {
            $line = $this->mainmodel->retRetrieveEmptyNewArray()[0];
            $line['cd_project_model'] = $cd_project_model;
            $line['ds_send_to'] = '';
...
// attachment
        ...
        $tableinfo = $this->docrep->getTableInfo(7);
        $aattach = $this->docrep->retSQLArray(" AND cd_project_comments = $cd_project_comment", $orderby = 'ORDER BY dt_record', $tableinfo);
        $grid->resetGrid();
        ...
        $this->load->view("tti/project_comment_form_view", $trans + $line + array(
            'toolbar' => $toolbar,
            'action' => $action,
            'cd_project_comments_answer_data' => $cd_project_comments_answer,
            'cd_user' => $this->session->userdata('cd_human_resource'),
            'ds_user' => $this->session->userdata('ds_human_resource_full'),
            'cd_project_model' => $cd_project_model,
