...
(function (factory) {
    ...
    PopoverX.prototype = $.extend({}, $.fn.modal.Constructor.prototype, {
        ...
        show: function (skipValidation) {
            ...
            var self = this, $dialog = self.$element;
            $dialog.css({top: 0, left: 0, display: 'block', 'z-index': self.options.zIndex}).appendTo(self.$body);
            ...
            $('.wrapper').LoadingOverlay("show", {zIndex: self.options.zIndex - 10, color: "rgba(200, 200, 200, 0.5)"});

        },
        destroy: function() {
            var self = this;
            self.$element.off(NAMESPACE);            
            $(self.$element).removeData('popover-x');
    ...
    $.fn.popoverX = function (option) {
        ...
        return self.each(function () {
            ...
            if (!options.$target) {
                if (data && data.$target && false) {
                    options.$target = data.$target;
                } else {
                    options.$target = $(option.target);
                }
            }
