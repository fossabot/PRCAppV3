...
(function () {
    var w2grid = function(options) {
        ...
        this.showNewWhenFiltering = true;
    ...
    w2grid.prototype = {
        ...
        add: function (record, first) {
            ...
            for (var o in record) {
                ...
                if (record[o].recid == null || typeof record[o].recid == 'undefined') {
                    console.log('ERROR: Cannot add record without recid. (obj: '+ this.name +')', record[o]);
                    continue;
                }
        ...
        localSort: function (silent) {
            ...
            this.records.sort(function (a, b) {
                ...
                for (var s in obj.sortData) {
                    ...
                    if (fld.substring(0,3) == 'dt_') {
                        aa = chkUndefined(aa, '');
                        bb = chkUndefined(bb, '');  
                        if (aa != '') {
                            aa = moment(aa.substring(0,10), defaultDateFormatUpper).format('YYYYMMDD');
                        } else {
                            aa = '19800101';
                        }
                        
                        if (bb != '') {
                            bb = moment(bb.substring(0,10), defaultDateFormatUpper).format('YYYYMMDD');
                        } else {
                            bb = '19800101';
                        }
        ...
        localSearch: function (silent) {
            ...
            if (this.searchData.length > 0 && !url) {
                ...
                for (var r in this.records) {
                    ...
                    for (var s in this.searchData) {
                        ...
                        //console.log('search', r, this.records[r].recid,this.isNewRow(this.records[r].recid) );
                        ...
                        if (this.isNewRow(this.records[r].recid) && this.showNewWhenFiltering) {
                            //this.last.searchIds.push(parseInt(r));
                            //continue;
                            fl++;
                        }
            ...
            //console.log('final', this.last.searchIds);
        ...
        editField: function (recid, column, value, event) {
            ...
            var boxrow = $('#grid_'+ obj.name +'_rec_'+ w2utils.escapeId(recid));
            var box    = boxrow.find('[col='+ column +']').find('div');
            ...
            var eventData = obj.trigger({ phase: 'before', type: 'editField', target: obj.name, recid: recid, column: column, value: value,
                index: index, originalEvent: event, box: box });
            if (eventData.isCancelled === true) return;
        ...
        editChange: function (el, index, column, event) {
            ...
            var isAlreadyChanged = (rec.changes && rec.changes[col.field] != undefined);
            ...
            while (true) {
                ...
                // CGB
                if ((typeof new_val != 'object' && ( String(old_val) != String(new_val) || isAlreadyChanged)) || 
        ...
        click: function (recid, event) {
            ...
            if (typeof event == 'undefined') event = {};
                ...
                if ( event.target != undefined &&  $(event.target).hasClass('cgbSlider')){
                return;
            }
            ...
            // event before
            var eventData = this.trigger({ phase: 'before', target: this.name, type: 'click', recid: recid, column: column, originalEvent: event, columnSelector: $(tmp) });
            if (eventData.isCancelled === true) return;
...
//                                            '   onpaste="var obj = this; setTimeout(function () { w2ui[\''+ obj.name + '\'].paste(obj.value); }, 1);" '+
        ...
        dblClick: function (recid, event) {
            //console.log('rodou o doubleclick');
            ...
            // event before
            var eventData = this.trigger({ phase: 'before', target: this.name, type: 'dblClick', recid: recid, column: column, originalEvent: event, columnSelector: $(tmp)  });
            if (eventData.isCancelled === true) return;
            // default action
            this.selectNone();
            var col = this.columns[column];
        ...
        render: function (box) {
            ...
            $(this.box)
                ...
                .html('<div>'+
                      ...
                      '    <div div id="grid_'+ this.name +'_toolbarScroll" style="display:block"><div id="grid_'+ this.name +'_toolbar" class="w2ui-grid-toolbar"></div></div>'+
            ...
            if ($('.w2ui-layout').length == 0) { // if there is layout, it will send a resize event
                ...
                if (w2ui[obj.name] === undefined) {
                    //console.log('error Resize', obj.name);
                };
            ...
            //$('#grid_'+ this.name +'_toolbarScroll').cgbMakeScrollbar({alwaysVisible: false, autoWrapContent: false, axis: 'x', height: '30px'});
        ...
        resizeRecords: function () {
            ...
            records.removeClass ('cgbGridRecordsY');                
            records.removeClass ('cgbGridRecordsYX');
            records.removeClass ('cgbGridRecordsX');
            ...
            if (bodyOverflowX || bodyOverflowY) {
                ...
                // cgb: Era "overflow-y": (bodyOverflowY ? 'auto' : 'hidden') });
                                
                  //records.slimScroll({height: '100%'});
        ...
        getCellHTML: function (ind, col_ind, summary) {
            ...
            if (col.render != null) {
                ...
                if (edit && ['checkbox', 'check'].indexOf(edit.type) != -1) {
                    ...
                    addStyle = 'text-align: center;padding: 0px 0px 0px 0px;';
                    
                    data = '<div class="checkbox checkbox-slider--b-flat" style="width: 40px;margin-left: auto; margin-right: auto;margin-top: 0px;margin-bottom:0px;"><label><input type="checkbox" '+ (data ? 'checked' : '') +' onclick="' +
                           '    var obj = w2ui[\''+ this.name + '\']; '+
                           '    obj.editChange.call(obj, this, '+ changeInd +', '+ col_ind +', event); ' +
                           ' "><span class="cgbSlider"></span></label></div>';
                   */
    ...
    w2toolbar.prototype = {
        ...
        render: function (box) {
            ...
            for (var i = 0; i < this.items.length; i++) {
                ...
                this.items[i]['htmlId'] = 'tb_'+ this.name + '_item_'+ it.id;
        ...
        getItemHTML: function (item) {
            ...
            switch (item.type) {
                ...
                case 'drop':
                    ...
                    html += '<table cellpadding="0" cellspacing="0" title="'+ item.hint +'" class="w2ui-button w2gridToolTip '+ (item.checked ? 'checked' : '') +'" '+
        ...
        click: function (id, event) {
            ...
            if (it && !it.disabled) {
                ...
                //$(obj.box).find('.w2gridToolTip').tooltip("hide");
        ...
        refresh: function () {
            ...
            for (var f in this.fields) {
                ...
                $(field.$el).off('change').on('change', function () {
                    ...
                    var value_previous = obj.record[this.name] ? obj.record[this.name] : '---------<EMPTY>*******';
